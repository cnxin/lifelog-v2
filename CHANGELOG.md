...
### UI 修复
- ✅ 修复未开启应用内暗色模式时，Android 系统暗色/强制深色可能让原生窗口底色变黑的问题。
- ✅ Android 启动背景和运行窗口背景固定为浅色，并禁用系统 `forceDark`。
- ✅ 修复暗色模式首页背景接近纯黑、部分文字对比度不足的问题。
- ✅ 暗色背景改为深蓝灰底色叠加低强度渐变，保留层次但不影响阅读。
- ✅ 暗色卡片、弹窗和页面主题统一使用同一套深色颜色。
- ✅ 补齐人物详情页和地点表单页的暗色模式传递。
- ✅ 版本更新为 `0.3.3+47`。
...
#### v0.3 开发方向：关系维护与数据可靠性
- ✅ **首页关系维护看板**：根据最近互动时间、联系间隔和共同回忆，自动列出需要关注的人。
- ✅ **人物关系摘要**：人物详情页展示最近互动、互动次数、关系状态、行动建议和常去地点。
- ✅ **备份健康信息**：账号页展示当前照片索引、地点合并历史和备份 schema 信息。
- ✅ **导入预览**：从剪贴板恢复前先解析备份，展示人物、地点、回忆、照片索引、设置、提醒和合并历史数量。
- ✅ **备份校验提示**：读取备份 integrity 字段，发现数量不一致时在导入确认页提示。

### 技术实现
- 新增 `relationship_insights.dart`，集中计算关系状态、互动次数、最近互动和常去地点。
- 新增 `backup_preview.dart`，集中解析备份 JSON、生成导入预览和完整性提示。
- 新增关系维护与备份预览单元测试。
- 开发阶段继续保留通用 APK 策略，优先兼容更多 Android 设备；上架前再改用 AAB / ABI 分包优化体积。
