...
- 主题系统：多套配色、可选 Android 动态取色、暗色模式、Glass UI 卡片和渐变背景。
...
| 农历 | lunar |
| 主题动态取色 | dynamic_color + Material 3 ColorScheme |
...
```text
releases/lifelog-v0.3.4+48.apk
```
...
- 主题系统短期保留现有 AppColors，动态取色作为设置项可选启用；后续逐步把页面组件迁移到 `Theme.of(context).colorScheme`。
- Android 原生层显式关闭 `forceDarkAllowed`，并补充 v27 样式配置，避免系统暗色或厂商强制深色把窗口背景染黑。
