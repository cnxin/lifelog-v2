...
class MemoryDetailPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final colors = AppColors.fromStyle(ref.watch(themeStyleProvider), isDark: isDark);
    final memory = memories.where((m) => m.id == memoryId).firstOrNull;

    if (memory == null) {
      return GradientBackground(colors: colors, isDark: isDark, child: const Scaffold(body: Center(child: Text('记忆不存在'))));
    }
            ...
            SliverPadding(
              ...
              sliver: SliverList(
                delegate: SliverChildListDelegate([
                  Container(
                    height: 120,
                    decoration: BoxDecoration(borderRadius: BorderRadius.circular(28), gradient: colors.primaryGradient, boxShadow: [colors.avatarShadow]),
                    child: const Icon(Icons.auto_stories_rounded, color: Colors.white, size: 46),
                  ),
                  const SizedBox(height: 20),
                  Text(title, style: TextStyle(fontFamily: 'Outfit', fontSize: 26, fontWeight: FontWeight.w700, color: colors.textMain)),
                  const SizedBox(height: 18),
                  _InfoCard(title: '基础信息', icon: Icons.info_rounded, colors: colors, child: _MemoryMeta(date: memory.date, people: personNames, placeName: place?.name ?? '', colors: colors)),
                  ...
                  if (content.isNotEmpty) _InfoCard(title: '内容', icon: Icons.notes_rounded, colors: colors, child: Text(content, style: TextStyle(color: colors.textMain, height: 1.55))),
                  if (tags.isNotEmpty) _InfoCard(title: '标签', icon: Icons.sell_rounded, colors: colors, child: Wrap(spacing: 6, runSpacing: 6, children: tags.map((tag) => _Tag(label: tag, colors: colors, accent: tag == memory.mood)).toList())),
                ]),
              ),
            ),
          ],
  ...
  void _delete(BuildContext context, WidgetRef ref, MemoryEvent memory) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text('确定要删除 ${memoryDisplayTitle(memory.title, memory.content)} 吗？'),
        actions: [
...
class _MemoryMeta extends StatelessWidget {
  ...
  const _MemoryMeta({required this.date, required this.people, required this.placeName, required this.colors});

  @override
  Widget build(BuildContext context) {
    final rows = [
      _MetaRow(icon: Icons.calendar_today_rounded, label: '日期', value: date, colors: colors),
      if (people.isNotEmpty) _MetaRow(icon: Icons.people_rounded, label: '人物', value: people.join('、'), colors: colors),
      if (placeName.isNotEmpty) _MetaRow(icon: Icons.place_rounded, label: '地点', value: placeName, colors: colors),
    ];
    return Column(children: rows.expand((row) => [row, if (row != rows.last) const SizedBox(height: 10)]).toList());
  }
}
...
class _MetaRow extends StatelessWidget {
  ...
  const _MetaRow({required this.icon, required this.label, required this.value, required this.colors});
  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: [
        ...
        SizedBox(width: 42, child: Text(label, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: colors.textSub))),
        Expanded(child: Text(value, style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: colors.textMain))),
      ],
    );
  }
}
...
class _Tag extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Text(label, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: accent ? const Color(0xFFE17055) : colors.primary)),
    );
  }
}
